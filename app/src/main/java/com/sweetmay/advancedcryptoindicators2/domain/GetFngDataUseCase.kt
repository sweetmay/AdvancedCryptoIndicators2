...
class GetFngDataUseCase(val fngRepo: IFnGRepo, val limit: String) : IGetFngDataUseCase {
  override suspend fun getFngView(): MainListViewState<FnGView> {
    return when (val result = fngRepo.getFng(limit)) {
      is ResultWrapper.Success -> {
        convertToViewState(result.value)
      }
      is ResultWrapper.Error -> {
        MainListViewState.Error(result.errorMsg)
      }
    }
  }

  private fun convertToViewState(fnGEntity: FnGEntity): MainListViewState<FnGView> {
    val data = fnGEntity.data
    return MainListViewState.Success(FnGView(
